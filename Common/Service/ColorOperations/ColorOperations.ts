import {IColorOperations} from "./Interface/IColorOperations";
...
export class ColorOperations implements IColorOperations {
	private static readonly MAX_RGB_VALUE: number = 255;
	private static readonly CIRCLE_DEGREES: number = 360;
	private static readonly RADIX: number = 16;
	private static readonly WHITE_HEX: number = 0xFFFFFF;

	public isLight (color: string, luminance: number = 60): boolean {
		const lum = parseFloat(this.hslStringToHslTuple(this.toHsl(color))[2]);
		return lum > luminance;
	}

	public lighten (color: string, percentage: number = 10): string {
		if (this.isRGBA(color)) return this.lightenRgba(color, percentage);
		if (this.isHSLA(color)) return this.lightenHsla(color, percentage);
		if (this.isRGB(color)) return this.lightenRgb(color, percentage);
		if (this.isHSL(color)) return this.lightenHsl(color, percentage);
		if (this.isHSV(color)) return this.lightenHsv(color, percentage);
		else return this.lightenHex(color, percentage);
	}

	public saturate (color: string, percentage: number = 10): string {
		if (this.isRGBA(color)) return this.saturateRgba(color, percentage);
		if (this.isHSLA(color)) return this.saturateHsla(color, percentage);
		if (this.isRGB(color)) return this.saturateRgb(color, percentage);
		if (this.isHSL(color)) return this.saturateHsl(color, percentage);
		if (this.isHSV(color)) return this.saturateHsv(color, percentage);
		else return this.saturateHex(color, percentage);
	}

	public takeInner (color: string): string {
	...
	public randomHslColor (): string {
		return this.hexToHsl(this.randomHexColor());
	}

	public randomHsvColor (): string {
		return this.rgbToHsv(this.randomRgbColor());
	}

	public toHex (color: string): string {

		if (this.isHex(color)) {
			if (color.length !== 4 && color.length !== 7) throw TypeError(`first argument to 'toHex()' must be a valid color. What appears to be a hex color was given: '${color}', but it doesn't have a valid amount of characters!`);
			return color;
		}
		...
		if (colorNameMatch) return colorNameMatch;

		else if (/^[0-9A-F]{3}$|^[0-9A-F]{6}$/i.test(color)) {
			// Must be a hex color without a '#' in front of it.
			if (color.length !== 3 && color.length !== 6) throw TypeError(`first argument to 'toHex()' must be a valid color. '${color}' was given!`);
			return `#${color}`;
		}

		else throw TypeError(`Couldn't decode color for input: ${color}`);
	}

	public toRgb (color: string): string {
		...
		const colorNameMatch = COLOR_NAMES[color.toLowerCase()];
		if (colorNameMatch) return this.hexToRgb(colorNameMatch);
	...
	public toHsl (color: string): string {
		...
		const colorNameMatch = COLOR_NAMES[color.toLowerCase()];
		if (colorNameMatch) return this.hexToHsl(colorNameMatch);
	...
	public toHsv (color: string): string {
		...
		const colorNameMatch = COLOR_NAMES[color.toLowerCase()];
		if (colorNameMatch) return this.rgbToHsv(this.hexToRgb(colorNameMatch));
	...
	public randomHexColor (): string {
		...
		if (color.length < 6) return this.randomHexColor();
		else return `#${color}`;
	}
	...
	private isHSLA (color: string): boolean {
		return color.slice(0, 4) === "hsla";
	}

	private isHSV (color: string): boolean {
	...
	private isRGBA (color: string): boolean {
	...
	*/
	...
	private hslStringToHslTuple (hsl: string): [number, string, string] {
		...
		if (isNaN(HUE)) throw TypeError(`Couldn't decode the 'hue' value for the given hsl color: ${hsl}`);
		if (isNaN(parseInt(SATURATION))) throw TypeError(`Couldn't decode the 'saturation' value for the given hsl color: ${hsl}`);
		if (isNaN(parseInt(LIGHTNESS))) throw TypeError(`Couldn't decode the 'lightness' value for the given hsl color: ${hsl}`);
	...
	private hslaStringToHslaTuple (hsla: string): [number, string, string, number] {
		...
		if (isNaN(ALPHA)) throw TypeError(`Couldn't decode the 'alpha' value for the given hsla color: ${hsla}`);
	...
	private hslToRgbTuple (hsl: string, rounding: boolean = true): [number, number, number] {
		...
		if (rounding) return [Math.round(r * ColorOperations.MAX_RGB_VALUE), Math.round(g * ColorOperations.MAX_RGB_VALUE), Math.round(b * ColorOperations.MAX_RGB_VALUE)];
		else return [r * ColorOperations.MAX_RGB_VALUE, g * ColorOperations.MAX_RGB_VALUE, b * ColorOperations.MAX_RGB_VALUE];
	}

	private hslToRgb (hsl: string, rounding: boolean = true): string {
		const [R, G, B] = this.hslToRgbTuple(hsl, rounding);
		return `rgb(${R}, ${G}, ${B})`;
	}

	private hslaToRgbaTuple (hsla: string): [number, number, number, number] {
	...
	private hslaToRgba (hsla: string): string {
		const [R, G, B, A] = this.hslaToRgbaTuple(hsla);
		return `rgba(${R}, ${G}, ${B}, ${A})`;
	}

	private hslaToHsl (hsla: string): string {
		const [H, S, L] = this.hslaStringToHslaTuple(hsla);
		return `hsl(${H}, ${S}, ${L})`;
	}

	private rgbStringToRgbTuple (rgbString: string): [number, number, number] {
		...
		if (isNaN(r)) throw TypeError(`'rgbToString()' couldn't decode the 'red' value of the given RGB color: '${rgbString}'.`);
		if (isNaN(g)) throw TypeError(`'rgbToString()' couldn't decode the 'green' value of the given RGB color: '${rgbString}'.`);
		if (isNaN(b)) throw TypeError(`'rgbToString()' couldn't decode the 'blue' value of the given RGB color: '${rgbString}'.`);

		return [r, g, b];
	}

	private rgbaStringToRgbaTuple (rgbaString: string): [number, number, number, number] {
		...
		if (isNaN(r)) throw TypeError(`Couldn't decode the 'red' value of the given RGBa color: '${rgbaString}'.`);
		if (isNaN(g)) throw TypeError(`Couldn't decode the 'green' value of the given RGBa color: '${rgbaString}'.`);
		if (isNaN(b)) throw TypeError(`Couldn't decode the 'blue' value of the given RGBa color: '${rgbaString}'.`);
		if (isNaN(a)) throw TypeError(`Couldn't decode the 'alpha' value of the given RGBa color: '${rgbaString}'.`);
	...
	private rgbaToHex (rgbaString: string, againstColor?: string) {
		...
		else return this.rgbToHex(`rgb(${r}, ${g}, ${b})`);
	}

	private hsvStringToHsvTuple (hsvString: string): [number, number, number] {
		...
		if (isNaN(h)) throw TypeError(`Couldn't decode the 'Hue' value of the given HSV color: '${hsvString}'.`);
		if (isNaN(s)) throw TypeError(`Couldn't decode the 'Saturation' value of the given HSV color: '${hsvString}'.`);
		if (isNaN(v)) throw TypeError(`Couldn't decode the 'Value/Brightness' value of the given HSV color: '${hsvString}'.`);

		return [h, s, v];
	}

	private hsvToRgbTuple (hsv: string): [number, number, number] {
	...
	*/
	...
	private hexToHslTuple (hex: string, rounding: boolean = true): [number, string, string] {
	...
	private rgbToHslTuple (rgb: string, rounding: boolean = true): [number, string, string] {
		...
		if (rounding) return [Math.round(h * ColorOperations.CIRCLE_DEGREES), `${Math.round(s * 100)}%`, `${Math.round(l * 100)}%`];
		else return [h * ColorOperations.CIRCLE_DEGREES, `${s * 100}%`, `${l * 100}%`];
	}
	...
	private rgbToHsvTuple (rgb: string): [number, number, number] {
	...
	private rgbToHsv (rgb: string): string {
	...
	private lightenRgba (rgba: string, percentage: number = 10): string {
		const [R, G, B, A] = this.rgbaStringToRgbaTuple(rgba);
		const [H, S, L] = this.rgbToHslTuple(`rgb(${R}, ${G}, ${B})`);
	...
	private saturateHex (hex: string, percentage: number = 10): string {
